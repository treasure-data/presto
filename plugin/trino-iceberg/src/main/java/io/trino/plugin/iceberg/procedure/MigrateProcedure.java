        ...
        implements Provider<Procedure>
{
    ...
    public void migrate(ConnectorSession session, String schemaName, String tableName, String recursiveDirectory)
    {
        ...
        try (ThreadContextClassLoader ignore = new ThreadContextClassLoader(getClass().getClassLoader())) {
    ...
    private static org.apache.iceberg.types.Type toIcebergType(Type type, AtomicInteger nextFieldId, IcebergFileFormat storageFormat)
    {
        if (type instanceof TinyintType || type instanceof SmallintType) {
            ...
            switch (storageFormat) {
                ...
                default:
                    throw new TrinoException(NOT_SUPPORTED, "Unsupported storage format for timestamp type: " + storageFormat);
            }
        }
        else if (type instanceof RowType rowType) {
            return fromRow(rowType, nextFieldId, storageFormat);
        }
        else if (type instanceof ArrayType arrayType) {
            return fromArray(arrayType, nextFieldId, storageFormat);
        }
        else if (type instanceof MapType mapType) {
            return fromMap(mapType, nextFieldId, storageFormat);
        }
        else {
    ...
    private static PartitionSpec parsePartitionFields(Schema schema, io.trino.plugin.hive.metastore.Table table)
    {
