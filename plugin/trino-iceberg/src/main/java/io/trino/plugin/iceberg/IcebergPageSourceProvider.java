        ...
        implements ConnectorPageSourceProvider
{
        ...
        if (!deletes.isEmpty()) {
            ...
            pageSource = TransformConnectorPageSource.create(pageSource, page -> {
                try {
                    ...
                    if (rowPredicate.isPresent()) {
                        page = rowPredicate.get().filterPage(page);
                    }
                    if (icebergColumns.size() == page.getChannelCount()) {
                        return page;
                    }
                    return new PrefixColumnsPage(page, icebergColumns.size()).getPage();
                }
    ...
    private record PrefixColumnsPage(Page page, int channelCount, int[] channels)
    {
        private PrefixColumnsPage
        {
            requireNonNull(page, "page is null");
            checkArgument(channelCount >= 0, "channelCount is negative");
            checkArgument(channelCount < page.getChannelCount(), "channelCount is greater than or equal to Page channel count");
            checkArgument(channels.length == channelCount, "channels length does not match channelCount");
        }

        private PrefixColumnsPage(Page page, int channelCount)
        {
            this(page, channelCount, IntStream.range(0, channelCount).toArray());
        }
