        ...
        implements ConnectorNodePartitioningProvider
{
            ...
            ConnectorPartitioningHandle partitioningHandle)
    {
        ...
        if (partitionFunctions.size() == 1 && partitionFunctions.get(0).transform() == BUCKET) {
            return Optional.of(createBucketNodeMap(partitionFunctions.get(0).size().orElseThrow())); //.withCacheKeyHint(handle.getCacheKeyHint()));
        }
    ...
    public ToIntFunction<ConnectorSplit> getSplitBucketFunction(
            ...
            ConnectorPartitioningHandle partitioningHandle)
    {
        return new IcebergBucketFunction((IcebergPartitioningHandle) partitioningHandle, typeOperators, 0 /* FIXME: bucketCount */);
    }
}
