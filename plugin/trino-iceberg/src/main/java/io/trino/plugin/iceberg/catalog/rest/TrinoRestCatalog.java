        ...
        implements TrinoCatalog
{
    ...
    {
        return listTablesInternal(session, namespace).stream()
                .map(TableInfo::tableName)
                .collect(toImmutableList());
    }

    //@Override
    private List<TableInfo> listTablesInternal(ConnectorSession session, Optional<String> namespace)
    {
    ...
    public Optional<ConnectorViewDefinition> getView(ConnectorSession session, SchemaTableName viewName)
    {
        return getIcebergView(session, viewName, false).flatMap(view -> {
            ...
            return Optional.of(new ConnectorViewDefinition(sqlView.sql(), catalog, schema, viewColumns, comment, owner, owner.isEmpty()));
        });
    }
    ...
    private Namespace findRemoteNamespace(ConnectorSession session, Namespace trinoNamespace)
    {
        ...
        return matchingRemoteNamespaces.isEmpty() ? trinoNamespace : matchingRemoteNamespaces.get(0);
    }
