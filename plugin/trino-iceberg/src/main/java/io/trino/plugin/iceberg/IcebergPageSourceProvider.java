...
import io.trino.parquet.reader.ParquetReaderNew;
...
import io.trino.plugin.hive.parquet.ParquetPageSourceNew;
...
import static io.trino.plugin.iceberg.IcebergUtil.createNullBlock;
import static io.trino.plugin.iceberg.IcebergUtil.deserializePartitionValue;
import static io.trino.plugin.iceberg.IcebergUtil.fromFieldBlocks;
import static io.trino.plugin.iceberg.IcebergUtil.getColumnHandle;
import static io.trino.plugin.iceberg.IcebergUtil.getDomain;
        ...
        implements ConnectorPageSourceProvider
{
            ...
            DynamicFilter dynamicFilter)
    {
        if (connectorSplit instanceof FilesTableSplit filesTableSplit) {
            return new FilesTablePageSource(
                    ...
                    fileIoFactory,
                    columns.stream().map(SystemColumnHandle.class::cast).map(SystemColumnHandle::getColumnName).collect(toImmutableList()),
                    filesTableSplit);
        }
        ...
        if (!deletes.isEmpty()) {
            ...
            pageSource = TransformConnectorPageSource.create(pageSource, page -> {
                try {
                    ...
                    return new PrefixColumnsSourcePage(page, icebergColumns.size()).getPage();
                }
            ...
            TupleDomain<IcebergColumnHandle> fileStatisticsDomain)
    {
        ...
        return unenforcedPredicate
                ...
                .filter((columnHandle, ignore) -> !partitionKeys.containsKey(columnHandle.getId()))
                // remove domains from predicate that fully contain split data because they are irrelevant for filtering
                .filter((handle, domain) -> !domain.contains(getDomain(fileStatisticsDomain, handle, domain.getType())));
    }
        ...
        try {
            ...
            for (IcebergColumnHandle column : columns) {
                if (column.isIsDeletedColumn()) {
                    ...
                    transforms.constantValue(createNullBlock(column.getType()));
                }
                else {
            ...
            Map<Integer, Optional<String>> partitionKeys)
    {
        ...
        try {
            ...
            ParquetReaderNew parquetReader = new ParquetReaderNew(
            ...
            ConnectorPageSource pageSource = new ParquetPageSourceNew(parquetReader);
            pageSource = transforms.build(pageSource);
            ...
            implements ProjectedLayout
    {
        ...
        public static ProjectedLayout createProjectedLayout(OrcColumn root, List<List<Integer>> fieldIdDereferences)
        {
            ...
            Map<Integer, List<List<Integer>>> dereferencesByField = fieldIdDereferences.stream()
                    .collect(groupingBy(items -> items.get(0), mapping(sequence -> sequence.subList(1, sequence.size()), toUnmodifiableList())));
    ...
    {
        private static Function<Page, Block> create(Slice filePath, int partitionSpecId, Slice partitionData)
        {
        ...
        public Block apply(Page page)
        {
            ...
            return fromFieldBlocks(rowPosition.getPositionCount(), fields);
        }
    }
    ...
    private record PrefixColumnsSourcePage(Page sourcePage, int channelCount, int[] channels)
    {
        ...
        private PrefixColumnsSourcePage(Page sourcePage, int channelCount)
        {
        ...
        {
            sourcePage.getPositions(positions, offset, size);
        }
    }
}
