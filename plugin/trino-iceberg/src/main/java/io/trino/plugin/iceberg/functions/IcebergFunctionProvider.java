        ...
        implements FunctionProvider
{
    public static final List<FunctionMetadata> FUNCTIONS = ImmutableList.<FunctionMetadata>builder()
            .add(FunctionMetadata.scalarBuilder()
            ...
            InvocationConvention invocationConvention)
    {
        ...
        checkArgument(argumentTypes.get(argumentTypes.size() - 1) == INTEGER, "The 2nd argument must be integer type, but got %s", argumentTypes.get(argumentTypes.size() - 1));
        Type type = argumentTypes.get(0);

        MethodHandle handle;
        if (type instanceof TinyintType || type instanceof SmallintType || type instanceof IntegerType || type instanceof BigintType) {
            handle = BUCKET_INTEGER;
        }
        else if (type instanceof DecimalType decimalType) {
            handle = decimalType.isShort() ? BUCKET_SHORT_DECIMAL : BUCKET_LONG_DECIMAL;
        }
        else if (type instanceof VarcharType) {
            handle = BUCKET_VARCHAR;
        }
        else if (type instanceof VarbinaryType) {
            handle = BUCKET_VARBINARY;
        }
        else if (type instanceof DateType) {
            handle = BUCKET_DATE;
        }
        else if (type instanceof TimestampType timestampType) {
            handle = timestampType.isShort() ? BUCKET_SHORT_TIMESTAMP : BUCKET_LONG_TIMESTAMP;
        }
        else if (type instanceof TimestampWithTimeZoneType timestampWithTimeZoneType) {
            handle = timestampWithTimeZoneType.isShort() ? BUCKET_SHORT_TIMESTAMP_WITH_TIME_ZONE : BUCKET_LONG_TIMESTAMP_WITH_TIME_ZONE;
        }
        else {
            throw new TrinoException(INVALID_FUNCTION_ARGUMENT, "Unsupported type: " + type);
        }
